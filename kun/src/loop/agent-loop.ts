...
const IMAGE_RECOGNITION_TOOL_NAME = 'recognize_image'
...
export function filterToolsForModelCapabilities<T extends ModelToolSpec>(
  toolSpecs: T[],
  modelCapabilities: ModelCapabilityMetadata
): T[] {
  if (!modelCapabilities.inputModalities.includes('image')) return toolSpecs
  return toolSpecs.filter((tool) => tool.name !== IMAGE_RECOGNITION_TOOL_NAME)
}
...
export class AgentLoop {
  ...
  private async modelStep(
    ...
    const tools = filterToolsForModelCapabilities(
