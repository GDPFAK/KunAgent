...
- For a new agent in this repository, prefer `âœ¨` over `ğŸ“š` because the change adds a new catalog capability rather than only updating existing documentation
...
| Bug Fix | `bugfix/JIRA-315-fix-token-refresh` | `ğŸ�› JIRA-315: fix token refresh race` | Non-production-critical defect work |
| Hotfix | `hotfix/JIRA-411-patch-auth-bypass` | `ğŸ�› JIRA-411: patch auth bypass check` | Production-critical fix from `main` |
...
| Docs | `feature/JIRA-623-document-api-errors` | `ğŸ“š JIRA-623: document API error catalog` | Documentation work with a Jira task |
| Tests | `bugfix/JIRA-724-cover-session-timeouts` | `ğŸ§ª JIRA-724: add session timeout regression tests` | Test-only change tied to a tracked defect or feature |
| Config | `feature/JIRA-811-add-ci-policy-check` | `ğŸ”§ JIRA-811: add branch policy validation` | Configuration or workflow policy changes |
| Dependencies | `bugfix/JIRA-902-upgrade-actions` | `ğŸ“¦ JIRA-902: upgrade GitHub Actions versions` | Dependency or platform upgrades |
...
- Repository-specific default: use `âœ¨` when adding a brand-new agent because Gitmoji defines it for new features; use `ğŸ“š` only when the change is limited to documentation updates around existing agents or contribution docs
...
commit_regex='^(ğŸš€|âœ¨|ğŸ�›|â™»ï¸�|ğŸ“š|ğŸ§ª|ğŸ’„|ğŸ”§|ğŸ“¦) [A-Z]+-[0-9]+: .+$'
...
## Planned Commits
1. ğŸ�› JIRA-315: fix refresh token race in auth service
2. ğŸ§ª JIRA-315: add concurrent refresh regression tests
3. ğŸ“š JIRA-315: document token refresh failure modes
...
- **cloudq**ï¼šå¤šäº‘ç®¡ç�†é¡¾é—® â€” å½“éœ€è¦�è¿›è¡Œå¤šäº‘ç»Ÿä¸€ç®¡ç�†ã€�æ�¶æ�„å�¯è§†åŒ–ã€�é£�é™©è¯„ä¼°æˆ– AI è¿�ç»´é—®ç­”æ—¶è‡ªåŠ¨è§¦å�‘
