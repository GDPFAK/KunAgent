...
export const BUILTIN_SUBAGENT_PROFILES: Readonly<Record<string, SubagentProfileConfig>> = {
  ...
  planner: {
    mode: 'subagent',
    toolPolicy: 'inherit',
    description: '规划代理:将复杂任务分解为可执行的子任务清单。',
    promptPreamble: '你是 Kun 的规划代理(Planner)。分析用户给的任务,将其分解为有序的子任务清单。每个子任务要独立可执行,标记依赖关系。'
  },
  researcher: {
    mode: 'subagent',
    toolPolicy: 'readOnly',
    description: '调研代理:搜索代码库、查找文档、回答技术问题,只读。',
    promptPreamble: '你是 Kun 的调研代理(Researcher)。你只读取文件、搜索代码库、查找文档。回答关于代码库的问题。高效、精准、只读。'
  },
  title: {
    mode: 'subagent',
    toolPolicy: 'readOnly',
    description: '标题生成:为会话生成简短的描述性标题。',
    promptPreamble: '为以下对话生成一个简短(不超过8个字)的描述性标题。只返回标题本身,不包含任何其他文字。'
  },
  summarizer: {
    mode: 'subagent',
    toolPolicy: 'readOnly',
    description: '会话摘要:压缩长对话上下文为结构化摘要。',
    promptPreamble: '为以上对话生成结构化的中文摘要。包含:已做事项、当前状态、关键决策与文件变更。保持简洁。'
  }
}
